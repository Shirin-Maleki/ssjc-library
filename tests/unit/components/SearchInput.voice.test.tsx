...
// These tests exercise voice → navigation behavior, not autocomplete — the server
// autocomplete action (Phase 5) is mocked to resolve with no suggestions so it never
// makes these tests depend on a real database.
vi.mock("@/lib/search/autocompleteAction", () => ({
  autocompleteAction: vi.fn().mockResolvedValue([]),
}));
