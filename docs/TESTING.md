...
| `adminReview.spec.ts` (Phase 8) | Against disposable fixtures created directly in the shared E2E database (never real Drive/library records): a `needs_review` ingestion item appears in the queue, opens, accepts a category choice, and approves into a record that is immediately conventionally searchable in Find; a same-edition duplicate decision adds a physical copy to the existing book and archives the pending placeholder, never creating a second active bibliographic record; a category referenced by an active book shows its real referencing count and a disabled Deactivate control rather than allowing a silent orphan |
