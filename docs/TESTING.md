...
| `googleSheets/rowBuilder.test.ts` (Phase 9, Location column added in the Phase 9 addendum) | `buildCoverCell`: a real `IMAGE()` formula only for a trusted https host, empty for an untrusted host/non-https/malformed URL/missing cover, no raw double quote from input ever survives into the formula. `buildCatalogRow`: exactly 17 columns (16 visible + 1 hidden), Location sits directly between Physical Category and Copy Count, no UUID/provenance/internal field name in the visible header; every ordinary text cell (Location included) is escaped with a leading `'` (formula-injection safety) unconditionally, including values that don't look dangerous; the cover column is the only formula-producing cell; copy count is sent as a real number; category shows the display label never the slug; language shows a friendly name never a raw code; age range/read time reuse Find's own formatters; genuinely unrecorded fields show "Not specified," never a fabricated value. `formatLocationSummary`: a bare label for one copy at one location, a count in parentheses for several at one location, every location listed with its own count for copies spread across several, and "Not specified" (never an empty string) when no copies are recorded at all |
...
| `db/bulkImport.test.ts` (Phase 9, `--location` threading added in the Phase 9 addendum) | Against real Postgres, with fully-scripted fake Drive/AI/metadata providers (zero real network calls): `createBulkImportJob` creates exactly one item per newly-discovered file in deterministic order, is idempotent on rerun (never a duplicate item for an already-tracked file), and respects an explicit `--limit`/`--file-id` bound; a confident item completes automatically creating exactly one active book + one copy; weak identification, a low-confidence category suggestion, and a real duplicate against an existing active book all correctly route to `needs_review` (the duplicate case never silently attaches a copy); a permanent provider failure (an empty/corrupt image) is marked `failed` without pointless retries; `recoverStaleProcessingItems` resets a genuinely stuck `processing` item back to `pending` after the staleness threshold and never touches one still within it; a bounded `runBulkImportJob` call processes exactly `maxItems` and a second call resumes and finishes the rest, with both real books ending up genuinely completed exactly once; a `needs_review` item appears in Phase 8's own computed `loadAdminReviewQueue` with zero new admin code; a job created with `initialLocationId` assigns that real location to the physical copy it creates, and a job created with no location leaves the copy's location genuinely unrecorded rather than guessed |
| `db/googleSheetsSync.test.ts` (Phase 9, Location column tests added in the Phase 9 addendum) | Against real Postgres, with a fully in-memory fake `SheetsProvider` (zero real network calls): the target spreadsheet is created exactly once and its identity persists in `system_settings`; a second call reuses it (never a duplicate); a spreadsheet deleted out-of-band (a real 404) is transparently replaced; sync includes only visible (`active`) books, excluding archived/pending_review; rerunning with no data changes is idempotent (same spreadsheet, same row count); a title/category change is reflected on the next sync; the visible catalog shrinking clears the correct number of stale trailing rows; `book_sheet_sync` rows are upserted for synced books and removed for books no longer visible; a trusted display cover produces a real `IMAGE()` formula while an untrusted/missing one produces an empty cell; no raw Drive source URL/file id ever appears in synced row data; a simulated Sheets API failure never mutates canonical book data and never marks a document "synced" that wasn't, and a retried sync after a failure succeeds cleanly. **Phase 9 addendum:** the Location column shows a bare label / a count / every location with its own count depending on how a book's real copies are distributed, and a real location change (the exact mutation `moveCopy()` performs) is reflected on the next sync |
| `db/locations.test.ts` (Phase 9 addendum) | Against real Postgres: `getCopyLocationSummary` for one copy/one location, several copies in one location, and copies spread across several locations plus a genuinely-unrecorded bucket (deterministic order, unrecorded bucket always last); `moveCopy` moves exactly one copy (source qty>1 decrements by one, destination increments by one), moves a copy out of the unrecorded bucket just as validly as a named one, records a truthful `audit_log` entry, rejects a nonexistent or inactive destination, rejects moving to the same location, fails honestly (`no_copy_available`) rather than corrupting state when no copy exists at the claimed source, never creates/destroys a physical copy or a bibliographic book during a move; two concurrent moves for the last copy at a location — exactly one succeeds, the other fails honestly, never both and never neither; location administration: `createLocation` generates a stable slug a later rename never changes, deactivation is blocked while copies are still recorded there and allowed once they're moved, and `moveCopy` itself refuses to assign a copy to an inactive destination |
...
| `moveBook.spec.ts` (Phase 9 addendum) | The side-effect-free half of Move/Return coverage, safe on every project concurrently: an unrecognizable cover shows a calm "couldn't match this photo to a book in the catalog" state, never a fabricated match, using the same `E2E_FAKE_INTAKE_PROVIDERS` fixture seam Add Book's own suite uses |
| `moveBookLocation.spec.ts` (Phase 9 addendum) | The real move-a-copy scenario: photo (the "gruffalo" fixture, matching the real seeded "The Gruffalo") → confirm the correct candidate by its real author (several same-titled-but-different-author books can legitimately exist in the shared E2E database by the time this runs — never assumed to be the only or first result) → choose the real seeded source location ("Forest Room") from a genuinely ambiguous multi-bucket list → choose a destination ("Blue Room") → the success screen shows the real from/to labels, and the database shows exactly the one previously-Forest-Room copy (identified by id, not by a hardcoded total count) now at Blue Room, with every other copy's location byte-for-byte unchanged. Runs desktop-only and inside `test.describe.serial()`, alone in its own file — this test mutates the real, shared seeded `book_copies` rows for "The Gruffalo," and both the cross-project (mobile vs. desktop) and cross-file race have to be eliminated the same way `teacherCatalog.spec.ts`'s does |
...
- Phase 5 real-provider validation (2026-09-17): captured and inspected real desktop (1440×900)
  ...
  phases).
- Phase 9 addendum: captured and inspected real screenshots (via another throwaway, deleted
  Playwright script run under the "mobile" project's real iPhone 13 viewport) of every Move/
  Return stage — capture, the "Is this it?" candidate confirmation, the source-location picker,
  the destination picker (confirming "Corridor 218" renders first as the pinned shortcut), and
  the success state. No layout defects found; the script's own real move was reversed
  afterward, leaving no lasting effect on shared seeded data.
